"""
Обучение честной модели с синтетическими xG признаками
Ожидаемая точность: 50-53%
"""
import json
import logging
import pickle
...
class SyntheticXGModelTrainer:
    def __init__(self, data_path: str = "data/historical/football_data_matches_with_xg.json"):
        self.data_path = Path(data_path)
        self.model_path = Path("data/models/model_synthetic_xg.pkl")
    ...
    def train(self):
        logger.info("🚀 Начинаю обучение модели с синтетическими xG")
        ...
        logger.info(f"✅ Точность: {accuracy:.4f} ({accuracy:.2%})")
        ...
        if accuracy >= 0.52:
            ...
            print("💎 VIP прогнозы будут иметь ~55-60%")
        elif accuracy >= 0.50:
            print("✅ Хорошо! Улучшение по сравнению с базовой 49%")
        else:
...
if __name__ == "__main__":
    trainer = SyntheticXGModelTrainer()
    trainer.train()
